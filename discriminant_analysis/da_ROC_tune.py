...
def main():
    ...
    ansLDA = []
    ansQDA = []
    for i in np.linspace(0.02, 0.17, 20):
        global t
        t = i
        print(t)
        _, _, AUC_LDA, AUC_QDA = ROC(dataset, num_tests=5)
        ts.append(i)
        ansLDA.append(AUC_LDA)
        ansQDA.append(AUC_QDA)
    
    plt.plot(ts, ansLDA, label="LDA")
    plt.plot(ts, ansQDA, label="QDA")
    plt.xlabel("t")
    plt.ylabel("AUC")
    plt.legend()
