...
def main():
    ...
    W3 = np.array(np.linalg.eig(matrix)[1][2], ndmin=2)
    ...
    ax.scatter([x[0] for x in new_liked], [x[1] for x in new_liked], [x[2] for x in new_liked], label="over_50k")
    ax.scatter([x[0] for x in new_disliked], [x[1] for x in new_disliked], [x[2] for x in new_disliked], alpha=0.7, label="under_50k")
